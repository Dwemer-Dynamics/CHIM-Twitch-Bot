...
if (!in_array($username, $lists['blacklist'])) {
    echo json_encode(['success' => true, 'message' => 'User was not in the blacklist']);
    exit;
}

// Remove from blacklist
$lists['blacklist'] = array_values(array_diff($lists['blacklist'], [$username]));
...
if ($success) {
    ...
    echo json_encode([
        'success' => true,
        'message' => "User '$username' has been removed from the blacklist",
        'lists' => $lists
    ]);
} else {
    echo json_encode([
